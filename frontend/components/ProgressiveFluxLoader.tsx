...
const DEFAULT_PHASES: ProgressiveFluxPhase[] = [
  ...
  { at: 15,  label: "initialising" },
  { at: 35,  label: "processing" },
  { at: 60,  label: "almost there" },
  { at: 85,  label: "finishing up" },
  { at: 100, label: "all done" },
];

// AmroGen brand: teal primary → sky accent. Override per-instance via the
// `--flux-from` / `--flux-to` CSS variables or the `gradient` prop.
const FLUX_FROM = "var(--flux-from, hsl(var(--primary)))";
const FLUX_TO   = "var(--flux-to,   hsl(var(--accent)))";
...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        <span className="inline-flex flex-wrap justify-center gap-x-[0.06em]">
